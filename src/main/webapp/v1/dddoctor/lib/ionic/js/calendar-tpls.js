        ...
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                scope.getHighlightClass = function (date) {
                    ...
                    if(rdate<nowDate){
                        className = 'monthview-oldDay';
                    }

                   var datelist = scope.$parent.$parent.$parent.dateList;
                   if(datelist){
                       var ttdate = dateFilter(rdate, 'yyyy-MM-dd');
                       if (datelist.indexOf(ttdate) >= 0) {
                           className = 'monthview-inList';
                       }
                   }

                    datelist = scope.$parent.$parent.$parent.notFullList;
                    if(datelist){
                        var ttdate = dateFilter(rdate, 'yyyy-MM-dd');
                        if (datelist.indexOf(ttdate) >= 0) {
                            className = 'monthview-inList';
                    ...
                    datelist = scope.$parent.$parent.$parent.fullList;
                    if(datelist){
                        var ttdate = dateFilter(rdate, 'yyyy-MM-dd');
                        if (datelist.indexOf(ttdate) >= 0) {

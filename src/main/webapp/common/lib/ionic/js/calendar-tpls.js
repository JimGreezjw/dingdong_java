        ...
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                scope.getHighlightClass = function (date) {
                    ...
                    var rdate = dateFilter(date.date, 'yyyy-MM-dd');
                    var nowdate = dateFilter(new Date(), 'yyyy-MM-dd');
                    if (rdate < nowdate) {
                        className = 'text-muted';
                    }

                    var schduleDateList = scope.$parent.$parent.$parent.schduleDateList;
                    if (schduleDateList) {
                        schduleDateList.forEach(function (item) {
                            if (item.scheduleDate == rdate) {
                                switch (item.status) {
                                    case 1:
                                        className = 'monthview-inList';
                                        break;
                                    case 2:
                                        className = 'monthview-fullList';
                                        break;
                                }
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                scope.getHighlightClass = function (date) {
                    ...
                    var datelist = scope.$parent.$parent.$parent.dateList;
                    var tdate = dateFilter(date, 'yyyy-MM-dd');
                    if (datelist.indexOf(tdate) >= 0) {
                        className = 'monthview-inList';
                    }
                    return className;
                };
...
angular.module("templates/rcalendar/day.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("templates/rcalendar/day.html",
        ...
        "                    <table class=\"table weekview-allday-content-table\">\n" +
        "                        <tbody>\n" +
        "                        <tr>\n" +
        ...
        "                <table class=\"table table-fixed dayview-normal-event-table\" ng-if=\"$index===currentViewIndex\">\n" +
        "                    <tbody>\n" +
...
angular.module("templates/rcalendar/month.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("templates/rcalendar/month.html",
        ...
        "<div class=\"row\">\n" +
        " <div class=\"col text-center\"> {{selectedDate.date | date:'yyyy-MM-dd' }}</div></div>\n" +
        "    <ion-slide-box class=\"monthview-slide\" on-slide-changed=\"slideChanged($index)\" does-continue=\"false\"\n" +
        "                   show-pager=\"true\">\n" +
        "        <ion-slide ng-repeat=\"view in views track by $index\">\n" +
        "            <table ng-if=\"$index===currentViewIndex\" class=\"table monthview-datetable\">\n" +
        "                <thead>\n" +
        "                <tr>\n" +
        "                    <th>\n" +
        "                        <small>日</small>\n" +
...
angular.module("templates/rcalendar/week.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("templates/rcalendar/week.html",
        "<div class=\"weekview\">\n" +
        "    <ion-slide-box class=\"weekview-slide\" on-slide-changed=\"slideChanged($index)\" does-continue=\"false\"\n" +
        "                   show-pager=\"false\">\n" +
        "        <ion-slide ng-repeat=\"view in views track by $index\">\n" +
        ...
        "                <tbody>\n" +
        "                <tr>\n" +
        "                    <th class=\"weekview-header text-center\" ng-repeat=\"dt in view.dates\"  ng-class=\"getHighlightClass(dt.date)\">{{::dt.date| date: 'd'}}</span></th>\n" +
        "                </tr>\n" +
        "                </tbody>\n" +
        "            </table>\n" +
        "        </ion-slide>\n" +
